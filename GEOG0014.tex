...
\emph{Please note that the \textbf{Select Features by Area or Single Click} button may be located in a different place than it is in the \textbf{GIF} below.}
...
We have been through the key steps that we need to create the map, however, there are some other things we can do. For instance, if
...
\tightlist
\item
  Using what you have learnt in the computer tutorial to create two heatmaps for \textbf{Camden and Islington}. The first of \textbf{anti-social behaviour} and the second of \textbf{theft from person}.
\item
