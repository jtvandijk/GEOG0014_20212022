...
Click on the three dots (**...**) next tot the **Vector Dataset(s)** box and navigate to your *working directory*. Then go into your `roads` folder and click on the roads file with the `SHP File` type. This is the roads `shapefile` that we want.
...
Click **Open**. Then click **Add**. If you get a messsage popping up saying something about "Select Transformations for raods" just click on **OK**. **Close** the dialog box. You should now see the road network of Camden and Islington loaded:
...
### Organising data {-}
Now we have our first set of data loaded, we can reorganise them a little. In the **Layers** panel, we can drag the `roads` layer on top of the `boroughs` layer. We can also zoom to a layer's full extent (right click on `layer` > **Zoom to Layer**), change change their colours (right click on `layer` > **Properties** > change the colour under the **Symbology** tab), and zoom in and out by scrolling:

```{r organise-data, echo=FALSE, fig.align='center', out.width='850pt', fig.cap='Boroughs loaded. [[Enlarge image]](https://jtvandijk.github.io/GEOG0014/images/w08/organise_data.gif){target="_blank"}'}
knitr::include_graphics('images/w08/organise_data.gif')
```

Before doing anything else, it would be a good idea to now save our QGIS project. Go to **File** > **Save** and save your file in your *working directory* as `crime_map`.
